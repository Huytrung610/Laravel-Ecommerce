<a class="tw-flex tw-items-center tw-text-third tw-relative" href="#" id="alertsDropdown" role="button"
   data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
    ...
    @if(count(auth()->user()->unreadNotifications) > 5 )<span data-count="5" id="notificationCount" class="count-notifications tw-absolute tw-text-third tw-bg-black tw-rounded-full tw-px-[5px] tw-py-[2px] tw-text-[11px] tw-top-[-7px] tw-right-[-12px]">5+</span>
    @else
        <span id="notificationCount" class="count-notifications tw-absolute tw-text-third tw-bg-black tw-rounded-full tw-px-[5px] tw-py-[2px] tw-text-[11px] tw-top-[-7px] tw-right-[-2px]"
            data-count="{{count(auth()->user()->unreadNotifications)}}">{{count(auth()->user()->unreadNotifications)}}</span>
    @endif

</a>
<!-- Dropdown - Alerts -->
<div class="dropdown-list dropdown-list-user dropdown-menu dropdown-menu-left shadow animated--grow-in tw-right-[-2px]"
