            ...
            <div class="new-address-member--form tw-flex tw-justify-center tw-hidden">
                <form action="{{route('address.add')}}"  method="POST" class="tw-w-4/6 tw-border tw-flex tw-flex-col tw-gap-3 tw-p-2">
                    ...
                    <div class="tw-flex tw-gap-3">
                        ...
                        <div class="tw-w-1/2">
                            <div data-mdb-input-init class="form-outline">
                                ...
                                <input type="text"  name="phone_number" value="" class="form-control phone_number_input" required/>
                                <span class="phone_error tw-hidden tw-text-red-400">Invalid phone number</span>
                            </div>
                        </div>
                    </div>
                    <div data-mdb-input-init class="form-outline">
                    ...
                    <div data-mdb-input-init class="form-outline">
                        ...
                        <input type="text" name="detail_address" value="" class="form-control"  required/>
                    </div>
                    <input type="hidden" id="name-user" class="form-control" name="user_id" value="" />
                    <button type="submit" class="btn btn-primary submit-address-btn">Add Address</button>
                </form>
            </div>
        </div>
            ...
            <div class="existing-address">
                <div class="address-item tw-flex tw-gap-2">
                    ...
                    <form method="POST" action="{{route('customer-address.destroy',[$addressItem->id])}}">
                        @csrf
                        @method('delete')
                        <button class="dlt-address--btn hover:tw-cursor-pointer" data-id="{{ $addressItem->id }}">
                ...
                <form action="{{ route('customer-address.update', ['id' => $addressItem->id]) }}" method="POST" class="tw-w-4/6 tw-flex tw-flex-col tw-gap-3 tw-p-2">
                    ...
                    <div class="tw-flex tw-gap-3">
                        ...
                        <div class="tw-w-1/2">
                            <div data-mdb-input-init class="form-outline">
                                <label class="form-label" for="phone-user">Phone</label>
                                <input type="text" name="phone_number" class="form-control phone_number_input" value="{{ $addressItem->phone_number }}" />
                                <span class="phone_error tw-hidden tw-text-red-400">Invalid phone number</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <button type="submit" class="btn btn-primary submit-address-btn">Update Address</button>
                </form>
            </div>
        </div>
    @endforeach
